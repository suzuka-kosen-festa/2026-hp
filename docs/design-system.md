...
トップで使う写真は、**グレースケール化してからテーマカラー1色にマッピング（デュオトーン）**する。フルカラーのまま置くと、コラージュの版画的な世界観から浮く。

割り当て: TIMETABLE=赤 / MAP=青 / BOOTH=黄 / ACCESS=青 / ヒーローの観客写真=赤。

手順（Pillow）:

```python
im = ImageOps.fit(im.convert("RGB"), (幅, 高さ))          # 表示比率で中央トリミング
g  = ImageOps.autocontrast(im.convert("L"), cutoff=2)     # 白飛び・黒潰れを整える
g  = g.point([min(255, int((i / 255) ** γ * 255)) for i in range(256)])  # γ>1 で中間調を沈める
g  = ImageEnhance.Contrast(g).enhance(1.2)
ImageOps.colorize(g, black=テーマカラー, white=(255, 255, 255)).save(出力, "WEBP", quality=85, method=6)
```

- **コントラストが出ないときは3点マップにする。** 暗部＝同系色の濃い版 / 中間＝テーマカラー / 明部＝白（`ImageOps.colorize(g, black=濃い色, mid=テーマカラー, white=白)`）。2点（テーマカラー↔白）だと**テーマカラーより暗い色が存在しない**ので、明るい色ほど眠くなる。とくに黄（#f4d21f）は2点では締まらない
  - 実際に使った暗部の色: 赤 `#6e0c10` / 黄 `#7a5f00`
  - **元写真が暗いときは、加工で粘らず明るい写真に差し替えるのが早い。** TIMETABLEは当初、暗い屋内ライブの写真を使っていて、暗部を持ち上げてもブランドの赤より明るい赤に振っても鮮やかにならなかった。屋外ステージの明るい写真に替えたら、標準の設定（暗部 `#6e0c10` ／ 中間 `#e31b23`）のまま解決した
  - 前処理は「下位5%を黒・上位95%を白に飛ばす（levels）→ S字カーブで中間を締める」。`autocontrast` だけだと元写真の明るさの差を吸収しきれない
  - **明るくしたいときに動かす順番**: ①白飛ばし側のパーセンタイルを下げる（97→82あたり）②γを1未満にして全体を持ち上げる（0.6〜0.9）③S字を弱める ④それでも重ければ暗部の色自体を明るくする。TIMETABLEの屋外ステージ写真は ①82 ②0.62 ③無効 ④`#a8121a` を使っている
- 青（#1a33d6）と、元から明暗のはっきりした写真は2点マップで足りる。γは 赤 1.1〜1.15 / 青 1.0〜1.3 が目安
- **ページ内のブランド赤より色あせて見えるときは、赤の範囲を広げてから彩度を足す。** 単純に色を変えるのではなく `colorize(..., midpoint=170)` でテーマカラーが占める階調を増やし、そのうえで `ImageEnhance.Color(...).enhance(1.25)`。デュオトーンは中間調が白寄りに散るため、既定のままだと隣のテープの赤より必ず淡く見える
- **写真にはごく弱いブラーをかける**（`GaussianBlur(1.0)`／2倍サイズの原寸に対して）。版画的なトーンに寄って、写真だけが浮くのを防ぐ。**図（MAP）にはかけない**（線が潰れる）。BOOTHは元画像が粗く、その粗さ自体が味になっているのでかけない
- 書き出しは **WebP**。`src/assets/` に置けば Astro が最終形をさらに最適化する
- **元画像は表示サイズの2倍**で用意し、`<Image>` に `densities={[1, 2]}` を付ける。付けないと指定幅ぶんしか生成されず、Retinaで甘くなる。`width` は**実際に表示される幅**に合わせること（ヒーローの写真は枠が320pxなのに245pxで指定されていた）
